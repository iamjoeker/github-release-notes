...
class Gren {
    ...
    async _getClosedIssues(releaseRanges) {
        ...
        const loadIssues = utils.task(this, `Getting all closed ${type}`);
        const { data: issues } = await this.issues.listIssues({
            state: 'closed',
            since: releaseRanges[releaseRanges.length - 1][1].date
        });

        loadIssues(`${type} found: ${issues.length}`);

        const loadCloseEvents = utils.task(this, `Checking close events for commits`);
        const issuesWithCommit = await Promise.all(issues
            .map(async issue => {
                const closeEvent = await this._getIssueCloseEvent(issue);

                return Object.assign(issue, { closing_commit_id: closeEvent.commit_id });
            })
        );

        // console.debug(issuesWithCommit);

        loadCloseEvents(`${type} closed by commit: ${issuesWithCommit.filter(i => i.closing_commit_id !== null).length}`);

        return issuesWithCommit;
    }
    ...
    _filterIssue(issue) {
        ...
        return (issue.pull_request ? dataSource === 'prs' : dataSource === 'issues' | dataSource === 'milestones') && !this._labelsAreIgnored(issue.labels) &&
    ...
    _filterPullRequest(pullRequest) {
        return !this._labelsAreIgnored(pullRequest.labels) && !(this.options.onlyMilestones && !pullRequest.milestone);
    }
     ...
     * @return {Boolean}
     */
    _filterBlockIssue(range, commits, issue) {
        ...
        return commits.some(c => c.sha === issue.closing_commit_id) ||
            utils.isInRange(
    ...
    async _getIssueBlocks(releaseRanges) {
        ...
        const release = await Promise.all(releaseRanges
            .map(async range => {
                const commits = await this._getCommitsBetweenTwo(range[1].date, range[0].date);

                const filteredIssues = Array.from(issues)
                    .filter(this._filterIssue.bind(this))
                    .filter(this._filterBlockIssue.bind(this, range, commits));
    ...
    async _getIssueCloseEvent(issue) {
        const { data: issueEvents } = await this.issues.listIssueEvents(issue.number);

        const closeEvent = issueEvents.filter(e => e.event === 'closed')[0];
